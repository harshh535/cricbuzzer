    ...
    # 3. Player Analytics
        ...
        if not df.empty:
            ...
            if not match_df.empty:
                ...
                with st.expander("Runs per Match"): st.line_chart(match_df["runs"])
                with st.expander("Wickets per Match"): st.line_chart(match_df["wickets"])
                with st.expander("Catches per Match"): st.line_chart(match_df["catches"])
                with st.expander("Efficiency Over Matches"): st.line_chart(match_df["efficiency"])
                with st.expander("Strike Rate Over Matches"): st.line_chart(match_df["strike_rate"])
                with st.expander("Economy Over Matches"): st.line_chart(match_df["economy"])
